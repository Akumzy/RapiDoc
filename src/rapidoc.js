...
export default class RapiDoc extends LitElement {
  ...
  connectedCallback() {
    ...
    if (
      ...
      !"example, model,".includes(`${this.defaultSchemaTab},`)
    ) {
    ...
    if (!this.ignoreRootPath) {
      this.ignoreRootPath = "false"
    }
    if (!this.routerMode) {
      this.routerMode = "hash"
    }
  ...
  // Nylas MOD
  fullPathname(path) {
    path = `${this.ignoreRootPath == "true" ? "" : this.rootPath}${
      path.startsWith("/") ? "" : "/"
    }${path}`
  ...
  async afterSpecParsedAndValidated(spec) {
    ...
    if (this.routerMode == "history") {
      if (window.location.pathname != `${this.rootPath}/`) {
        locationHash = window.location.pathname
      }
    }
        ...
        () => {
          ...
          if (gotoEl) {
            ...
            window.history.replaceState(
              null,
              null,
              `${this.routerMode == "hash" ? "#" : "/"}${this.fullPathname(
                `/${tmpElementId}`,
              )}`,
            )
          }
    ...
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.intersectionRatio > 0) {
        ...
        if (newNavEl) {
          const url =
            this.routerMode == "hash"
              ? `${window.location.href.split("#")[0]}#${entry.target.id}`
              : `/${this.fullPathname(`/${entry.target.id}`)}`
  ...
  async setFocusedElementId(elementId) {
  ...
  async scrollTo(elementId, expandPath = true, scrollNavItemToView = true) {
    this.setFocusedElementId(elementId)
    ...
    if (this.renderStyle === "view") {
      ...
      if (!contentEl) {
        elementId = elementId.startsWith(this.rootPath)
          ? elementId.replace(this.rootPath, "")
          : elementId
       await this.setFocusedElementId(elementId)
        elementId = elementId.startsWith("/") ? elementId.slice(1) : elementId
        contentEl = this.shadowRoot.getElementById(elementId)
        if (!contentEl) {
          elementId = elementId.endsWith("/")
            ? elementId.slice(0, -1)
            : elementId
         await this.setFocusedElementId(elementId)
          contentEl = this.shadowRoot.getElementById(elementId)
      ...
      if (contentEl) {
        ...
        if (
          this.renderStyle === "read" ||
          elementId.startsWith("overview") ||
          elementId.startsWith("tag--")
        ) {
      ...
      if (isValidElementId) {
        ...
        let path = `${this.routerMode == "hash" ? "#" : "/"}${this.fullPathname(
          `${elementId}`,
        )}`
